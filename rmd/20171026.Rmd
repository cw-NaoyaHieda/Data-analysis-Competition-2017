...
onaosi_id <- dataset4 %>% filter(category_1=="クーポン                          ") %>% select(product_id)

onaosi_line <- onaosi_id %>% left_join(dataset3,"product_id")

onaosi_data <- dataset2 %>% left_join(onaosi_line,"receipt_id") %>%
  mutate(coupon_ = !is.na(product_id)) %>% select(customer_id,coupon_) 

dataset2 <- dataset2 %>% left_join(onaosi_data,"customer_id")
...
```{r}
new_customer_firstdata$sum_coupon_num
```
...
new_customer_firstdata_naout <- na.omit(new_customer_firstdata %>% select(repeater,t,day,dm,sex,store_id,in_tax,regi_staff,simei,item_num,sum_coupon_num))
...
output.glm <- glm(repeater~ day + dm + sex +
                    store_id + in_tax_ + regi_staff + sum_coupon_num + 
                    simei + hour_ ,
