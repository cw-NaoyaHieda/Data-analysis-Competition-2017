...
FROM(
	...
	FROM(
		...
		WHERE item_treat = '施術販売' AND product_id != '70010001' AND product_id != '70010002' AND product_id != '70010003'
			...
			AND product_id != '10010013') AS B
		ON A.receipt_id = B.receipt_id
		WHERE (A.t BETWEEN '13:00:00' AND '18:00:00') AND A.day <= 4
		GROUP BY A.receipt_id) AS C
	GROUP BY C.customer_id) AS D
WHERE E.customer_id = D.customer_id);
...
FROM(
	...
	FROM(
		...
		WHERE item_treat = '施術販売' AND product_id != '70010001' AND product_id != '70010002' AND product_id != '70010003'
			...
			AND product_id != '10010013') AS B
		ON A.receipt_id = B.receipt_id
		WHERE A.t > '18:00:00' AND A.day <= 4
		GROUP BY A.receipt_id) AS C
	GROUP BY C.customer_id) AS D
WHERE E.customer_id = D.customer_id);

-- 空欄を0で埋める
UPDATE customer_2 SET morning_count_weekday=0 WHERE morning_count_weekday IS NULL;
UPDATE customer_2 SET evening_count_weekday=0 WHERE evening_count_weekday IS NULL;
UPDATE customer_2 SET night_count_weekday=0 WHERE night_count_weekday IS NULL;
